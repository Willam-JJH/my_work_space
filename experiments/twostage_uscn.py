...
us=pd.read_parquet('D:/code/data/us_returns.parquet')
cn=pd.read_parquet('D:/code/data/cn_returns.parquet')
us_comp=us.notna().sum()/len(us); us_top=us_comp.nlargest(50).index.tolist()
cn_comp=cn.notna().sum()/len(cn); cn_top=cn_comp.nlargest(50).index.tolist()
...
print(f'US:50 + CN:50 = {n_assets} stocks x {ret_vals.shape[0]} days')
